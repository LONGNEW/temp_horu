...
@dataclass
class FedAvgCNNMethod(NNFederatedMethod):
    num_classes: int
    local_epochs: int
    batch_size: int
    lr: float
    ...
    def client_step(self, client: ClientData, state: NNClientState) -> tuple[dict[str, Any], NNClientState]:
        ...
        materialize_model_ms = (time.perf_counter() - materialize_started) * 1000.0
        ...
        if self.debug:
            print(
                f"[debug][fedavg_cnn][round={self._round + 1}][client={client.client_id}] "
            ...
            self._current_round_system_rows.append(
                {
                    ...
                    "materialize_model_ms": materialize_model_ms,
                    "encode_train_ms": 0.0,
                    "local_update_ms": local_update_ms,
                    "fedavg_global_local_update_ms": local_update_ms,
