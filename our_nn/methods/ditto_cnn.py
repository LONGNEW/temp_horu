...
@dataclass
class DittoCNNMethod(NNFederatedMethod):
    num_classes: int
    local_epochs: int
    batch_size: int
    global_lr: float
    ...
    def __post_init__(self) -> None:
        ...
        self._last_server_stage_metrics: dict[str, float] = {
            "server_average_ms": 0.0,
            "server_apply_model_ms": 0.0,
        }
    ...
    def client_step(self, client: ClientData, state: NNClientState) -> tuple[dict[str, Any], NNClientState]:
        ...
        materialize_global_reference_ms = (time.perf_counter() - prox_started) * 1000.0

        global_materialize_started = time.perf_counter()
        global_local_model = self._new_model()
        global_local_model.load_state_dict(self.global_model.state_dict())
        global_model_materialize_ms = (time.perf_counter() - global_materialize_started) * 1000.0
        global_metrics = {"accuracy": 0.0}
        global_update_started = time.perf_counter()
        ...
        global_local_update_ms = (time.perf_counter() - global_update_started) * 1000.0
        ...
        personal_update_started = time.perf_counter()
        for _ in range(self.local_epochs):
        ...
        personal_local_update_ms = (time.perf_counter() - personal_update_started) * 1000.0

        if self.debug:
            print(
                f"[debug][ditto_cnn][round={self._round + 1}][client={client.client_id}] "
        ...
        next_state = NNClientState(personalized_state=personalized_state)
        payload_pack_ms = (time.perf_counter() - payload_pack_started) * 1000.0
        if self.enable_system_profiling and trace_round:
            payload_bytes = float(sum(t.numel() * t.element_size() for t in payload["model_state"].values()))
            self._current_round_system_rows.append(
                {
                    ...
                    "encode_train_ms": 0.0,
                    "local_update_ms": global_local_update_ms + personal_local_update_ms,
                    ...
                    "materialize_global_reference_ms": materialize_global_reference_ms,
                    "global_model_materialize_ms": global_model_materialize_ms,
                    "global_local_update_ms": global_local_update_ms,
    ...
    def server_step(self, payloads: list[dict[str, Any]]) -> None:
        ...
        if payloads:
            ...
            self._last_server_stage_metrics["server_average_ms"] = (time.perf_counter() - average_started) * 1000.0
            apply_started = time.perf_counter()
            self.global_model.load_state_dict(averaged_state)
            self._last_server_stage_metrics["server_apply_model_ms"] = (time.perf_counter() - apply_started) * 1000.0
